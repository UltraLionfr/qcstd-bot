...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!ticket) {
      return interaction.reply({ content: lang.ticket.not_in_ticket, flags: MessageFlags.Ephemeral });
    }
    ...
    // Log fermeture
    const logChannel = await interaction.client.channels.fetch(config.logsChannel).catch(() => null);
    if (logChannel) {
